...
bool analyzer::is_definable_func(const std::string&name)const
{
    return funcs.count(name)==0;
}
bool analyzer::is_definable_var(const std::string&name)const
{
    return vars.back().count(name)==0;
}
bool analyzer::is_breakable()const noexcept
{
    return break_nest>0;
}
bool analyzer::is_continuable()const noexcept
{
