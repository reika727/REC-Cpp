...
std::unique_ptr<const expression>expression::get_order15(lexicon::token_array&ta) // , left to right
{
    ...
    while(auto tp=ta.consume_symbol(lexicon::symbol::COMMA))
        ret=std::make_unique<const comma>(std::move(ret),get_order14(ta),tp.value().line,tp.value().col);
    return ret;
}
std::unique_ptr<const expression>expression::get_order14(lexicon::token_array&ta) // = += -= *= /= right to left
{
    ...
    while(true){
        if(auto tp=ta.consume_symbol(lexicon::symbol::EQUAL))
            ret=std::make_unique<const assign>(std::move(ret),get_order14(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::PLEQ))
            ret=std::make_unique<const plasgn>(std::move(ret),get_order14(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::MIEQ))
            ret=std::make_unique<const miasgn>(std::move(ret),get_order14(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::ASEQ))
            ret=std::make_unique<const muasgn>(std::move(ret),get_order14(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::SLEQ))
            ret=std::make_unique<const diasgn>(std::move(ret),get_order14(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::PEEQ))
            ret=std::make_unique<const rmasgn>(std::move(ret),get_order14(ta),tp.value().line,tp.value().col);
        else
            break;
    }
    return ret;
...
std::unique_ptr<const expression>expression::get_order12(lexicon::token_array&ta) // || left to right
{
    ...
    while(auto tp=ta.consume_symbol(lexicon::symbol::VBVB))
        ret=std::make_unique<const logor>(std::move(ret),get_order11(ta),tp.value().line,tp.value().col);
    return ret;
}
std::unique_ptr<const expression>expression::get_order11(lexicon::token_array&ta) // && left to right
{
    ...
    while(auto tp=ta.consume_symbol(lexicon::symbol::APAP))
        ret=std::make_unique<const logand>(std::move(ret),get_order10(ta),tp.value().line,tp.value().col);
    return ret;
}
...
std::unique_ptr<const expression>expression::get_order07(lexicon::token_array&ta) // == != left to right
{
    ...
    while(true){
        if(auto tp=ta.consume_symbol(lexicon::symbol::EQEQ))
            ret=std::make_unique<const equal>(std::move(ret),get_order06(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::EXEQ))
            ret=std::make_unique<const nequal>(std::move(ret),get_order06(ta),tp.value().line,tp.value().col);
        else
            break;
    }
    return ret;
...
std::unique_ptr<const expression>expression::get_order06(lexicon::token_array&ta) // < > <= >= left to right
{
    ...
    while(true){
        if(auto tp=ta.consume_symbol(lexicon::symbol::LESS))
            ret=std::make_unique<const less>(std::move(ret),get_order05(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::GREATER))
            ret=std::make_unique<const greater>(std::move(ret),get_order05(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::LEEQ))
            ret=std::make_unique<const leeq>(std::move(ret),get_order05(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::GREQ))
            ret=std::make_unique<const greq>(std::move(ret),get_order05(ta),tp.value().line,tp.value().col);
        else
            break;
    }
    return ret;
...
std::unique_ptr<const expression>expression::get_order04(lexicon::token_array&ta) // + - left to right
{
    ...
    while(true){
        if(auto tp=ta.consume_symbol(lexicon::symbol::PLUS))
            ret=std::make_unique<const bplus>(std::move(ret),get_order03(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::MINUS))
            ret=std::make_unique<const bminus>(std::move(ret),get_order03(ta),tp.value().line,tp.value().col);
        else
            break;
    }
    return ret;
...
std::unique_ptr<const expression>expression::get_order03(lexicon::token_array&ta) // * / % left to right
{
    ...
    while(true){
        if(auto tp=ta.consume_symbol(lexicon::symbol::ASTER))
            ret=std::make_unique<const multiply>(std::move(ret),get_order02(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::SLASH))
            ret=std::make_unique<const divide>(std::move(ret),get_order02(ta),tp.value().line,tp.value().col);
        else if(tp=ta.consume_symbol(lexicon::symbol::PERCENT))
            ret=std::make_unique<const remain>(std::move(ret),get_order02(ta),tp.value().line,tp.value().col);
        else
            break;
    }
    return ret;
...
{
    if(auto tp=ta.consume_symbol(lexicon::symbol::PLUS))
        return std::make_unique<const uplus>(get_order02(ta),tp.value().line,tp.value().col);
    else if(tp=ta.consume_symbol(lexicon::symbol::MINUS))
        return std::make_unique<const uminus>(get_order02(ta),tp.value().line,tp.value().col);
    else if(tp=ta.consume_symbol(lexicon::symbol::PLPL))
        return std::make_unique<const preinc>(get_order02(ta),tp.value().line,tp.value().col);
    else if(tp=ta.consume_symbol(lexicon::symbol::MIMI))
        return std::make_unique<const predec>(get_order02(ta),tp.value().line,tp.value().col);
    else if(tp=ta.consume_symbol(lexicon::symbol::EXCLAM))
        return std::make_unique<const lognot>(get_order02(ta),tp.value().line,tp.value().col);
    else
