    ...
    :statement(lx.get_line(),lx.get_column())
{
    ...
    if(!lx.consume_symbol(lexicon::symbol::SCOLON))
...
compound::compound(lexicon::lexer&lx)
    :statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::OBRACE))
        throw exception::compilation_error("複文の開始ブラケットが見つかりませんでした",lx.get_line(),lx.get_column());
    while(!lx.consume_symbol(lexicon::symbol::CBRACE))stats.push_back(statement::get(lx));
}
var_difinition::var_difinition(lexicon::lexer&lx)
    :statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::INT))
        ...
        if(auto idp=lx.consume_identifier())
            vars.push_back(
                std::make_pair(
                    ...
                    lx.consume_symbol(lexicon::symbol::EQUAL)?expression::get(lx,true):nullptr
                )
            );
        else
...
_if_else_::_if_else_(lexicon::lexer&lx)
    :statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::IF))
        throw exception::compilation_error("ifキーワードが見つかりませんでした",lx.get_line(),lx.get_column());
    if(!lx.consume_symbol(lexicon::symbol::OPARENT))
    ...
    stat_else=lx.consume_symbol(lexicon::symbol::ELSE)?statement::get(lx):nullptr;
}
_while_::_while_(lexicon::lexer&lx)
    :iteration_statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::WHILE))
...
_for_::_for_(lexicon::lexer&lx)
    :iteration_statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::FOR))
...
_break_::_break_(lexicon::lexer&lx)
    :statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::BREAK))
...
_continue_::_continue_(lexicon::lexer&lx)
    :statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::CONTINUE))
...
_return_::_return_(lexicon::lexer&lx)
    :statement(lx.get_line(),lx.get_column())
{
    if(!lx.consume_symbol(lexicon::symbol::RETURN))
    ...
    :node(lx.get_line(),lx.get_column())
{
    ...
    if(lx.consume_symbol(lexicon::symbol::VOID)){
        if(!lx.consume_symbol(lexicon::symbol::CPARENT))
        ...
        while(true){
            ...
            if(lx.consume_symbol(lexicon::symbol::COMMA))
                continue;
            else if(lx.consume_symbol(lexicon::symbol::CPARENT))
                break;
            else
                throw exception::compilation_error("不正な区切り文字です",lx.get_line(),lx.get_column());
        }
